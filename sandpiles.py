...
'''
If you find this, I was using this computer because it is beefy and needed to do some beefy computations

This models what is called an Abelian Sandpile 

Will somehow turn this into a neural network
...
class Sandpile():
    def __init__(self, N, threshold, IC):
        self.N = N
        self.thresh = threshold
        if IC == 1:
            self.grid = np.zeros((N, N))
            self.grid[int(N/2), int(N/2)]

    def draw_grid(self, grid, screen, cell_size):
        for i in range(N):
            for j in range(N):
    ...
    def play_animation(self, batchsize=1000):
        pygame.init()
        ...
        while running:
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    running = False
            batch_iterations(grid, n=batchsize)
            self.draw_grid(self, grid, screen, cell_size)
            pygame.display.flip()
            clock.tick(60)
            frame += 1
        pygame.quit()
